      ...
      <div className="w-full min-h-screen flex flex-col items-center px-5 lg:px-20 pt-20 pb-10">
        ...
        <div className="relative">
          <Image
            ...
            height={450}
            priority
            className="w-[250px] sm:w-[320px] md:w-[400px] lg:w-[450px] h-auto"
          />
        </div>

        {/* tarjeta */}
        <div className="w-full max-w-[700px] bg-white/90 dark:bg-slate-800 backdrop-blur-md rounded-[40px] shadow-xl p-6 sm:p-8 md:p-10 mt-4">
          ...
          <button
            ...
            className="w-full h-[58px] md:h-[65px] mt-8 rounded-2xl bg-gradient-to-r from-[#2563ff] to-[#18C3F7] text-white font-bold text-lg md:text-xl shadow-lg hover:scale-[1.02] transition"
          >
          ...
          <p className="text-center mt-8 text-gray-600 dark:text-gray-300">
            ¿No tienes cuenta?
            <Link href="/Registro" className="ml-2 font-bold text-blue-500">Regístrate</Link>
          </p>
        </div>
      </div>
    </main>
...
function InputBox({ icon, rightIcon, ...props }: any) {
  return (
    <div className="w-full h-[58px] md:h-[65px] border rounded-2xl px-5 flex items-center">
      <div className="text-blue-500 mr-4">{icon}</div>
      <input {...props} className="flex-1 bg-transparent outline-none text-base md:text-lg text-black dark:text-white placeholder:text-gray-500 dark:placeholder:text-gray-400" />
      {rightIcon}
    </div>
  );
}